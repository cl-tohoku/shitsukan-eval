...
TASK_DESCRIPTION_FOR_TAXONOMIC_A_B_CLASSIFICATION_JA = (
    "質感とは、下記のようなものを指します。\n"
    "・物性（光沢感・透明感など）\n"
    "・状態（乾燥・凍結など）\n"
    "・印象（美しい・醜いなど)\n"
    "以下の選択肢のうち、質感語として正しいものを1つ選び解答してください。\n"
    '解答の際には、選択肢に対応するアルファベット ("A" もしくは "B") だけを使って解答してください。'
)
...
def get_eval_scores(data_list: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    """
    label_to_id_dict = {"A": 0, "B": 1, "-100": -100}

    gold_list = []
    pred_list = []
    for data in data_list:
        gold_list.append(data["correct_ids"][0])
...
def taxonomic_a_b_classification_ja(
    model,
    ...
    >>> }
    >>> taxonomic_a_b_classification_ja(model, task_config, verbose=True, save_dir="results", task="perception", sub_task="selection", sub_subtask=None, lang="ja", model_name="gpt-4o")
    """
    new_data_list = []
    ...
    for data in tqdm(data_list, desc="Processing dataset", leave=False):
        ...
        data (Dict[str, Any]):
        {
            ...
            'uncorrect_shitsukan_list': ['淀みがない'],
            'correct_ids_with_shitsukan': ['B:睦まじい'],
            'uncorrect_ids_with_shitsukan': ['A:淀みがない'],
            'all_ids_with_shitsukan': ['A:淀みがない', 'B:睦まじい']
        }
        """
        ...
        system_prompt = TASK_DESCRIPTION_FOR_TAXONOMIC_A_B_CLASSIFICATION_JA
