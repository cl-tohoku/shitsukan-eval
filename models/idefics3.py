...
class Idefics3:
    ...
    Example usage:
    >>> model = Idefics3(pretrained="HuggingFaceM4/Idefics3-8B-Llama3", device_map={"": 0})
    ...
    def create_system_prompt(system_prompt: str) -> list[dict[str, Any]]:
        ...
        Example:
        >>> prompt = Idefics3.create_system_prompt("You are a helpful assistant.")
    ...
    def create_zero_shot_prompt(zero_shot_data: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        >>> prompt = Idefics3.create_zero_shot_prompt(zero_shot)
        """
        messages = []
        role = "user"
        content = []
    ...
    def create_few_shot_prompt(few_shot_data: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        >>> prompt = Idefics3.create_few_shot_prompt(few_shot)
        """
        messages = []
    ...
    def generate_response(
        ...
        if system_prompt:
            message += Idefics3.create_system_prompt(system_prompt)

        # Add few-shot examples if provided
        if few_shot_data:
            message += Idefics3.create_few_shot_prompt(few_shot_data)
        ...
        if images is not None:
            ...
            if img_token_num > len(images):
                ...
                message += Idefics3.create_zero_shot_prompt({"user": text, "images": None})
            else:
                add_img_token_num = len(images) - img_token_num
                # Add the main user input (text and images)
                message += Idefics3.create_zero_shot_prompt({"user": text, "images": images[:add_img_token_num]})
        else:
