...
class MllamaInstructHf:
    ...
    Example usage:
    >>> model = MllamaInstructHf(pretrained="meta-llama/Llama-3.2-11B-Vision-Instruct", device_map={"": 0})
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Initializes the MllamaInstructHf class by loading the model, processor, and tokenizer.
        ...
        Example:
        >>> model = MllamaInstructHf(pretrained="meta-llama/Llama-3.2-11B-Vision-Instruct", device="cuda")
        """
        super().__init__()
    ...
    def create_system_prompt(system_prompt: str) -> list[dict[str, Any]]:
        ...
        Example:
        >>> prompt = MllamaInstructHf.create_system_prompt("You are a helpful assistant.")
    ...
    def create_zero_shot_prompt(zero_shot_data: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        >>> prompt = MllamaInstructHf.create_zero_shot_prompt(zero_shot)
        """
        messages = []
        role = "user"
        content = []
    ...
    def create_few_shot_prompt(few_shot_data: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        >>> prompt = MllamaInstructHf.create_few_shot_prompt(few_shot)
        """
        messages = []
    ...
    def generate_response(
        ...
        Example usage:
        >>> model = MllamaInstructHf(pretrained="meta-llama/Llama-3.2-11B-Vision-Instruct")
        ...
        if system_prompt:
            message += MllamaInstructHf.create_system_prompt(system_prompt)

        # Add few-shot examples if provided
        if few_shot_data:
            message += MllamaInstructHf.create_few_shot_prompt(few_shot_data)

        # Add the main user input (text and images)
        message += MllamaInstructHf.create_zero_shot_prompt({"user": text, "images": images})
