...
class Qwen25InstructHf:
    """
    A wrapper class for using the Qwen2.5 models from Hugging Face with conditional generation.
    ...
    Example usage:
    >>> model = Qwen25InstructHf(pretrained="Qwen/Qwen2.5-7B-Instruct", device_map={"": 0})
    ...
    def __init__(
        self,
        pretrained: str = "Qwen/Qwen2.5-7B-Instruct",
        revision: str = "main",
        device: str = "cuda",
        ...
        **kwargs,
        ...
        """
        Initializes the Qwen25InstructHf class by loading the model, processor, and tokenizer.
        ...
        Example:
        >>> model = Qwen25InstructHf(pretrained="Qwen/Qwen2.5-7B-Instruct", device="cuda")
        """
        super().__init__()
    ...
    def create_system_prompt(system_prompt: str) -> list[dict[str, Any]]:
        ...
        Example:
        >>> prompt = Qwen25InstructHf.create_system_prompt("You are a helpful assistant.")
    ...
    def create_zero_shot_prompt(zero_shot_data: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        >>> prompt = Qwen25InstructHf.create_zero_shot_prompt(zero_shot)
        """
        role = "user"
    ...
    def create_few_shot_prompt(few_shot_data: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        >>> prompt = Qwen25InstructHf.create_few_shot_prompt(few_shot)
        """
        messages = []
    ...
    def generate_response(
        ...
        Example usage:
        >>> model = Qwen25InstructHf(pretrained="Qwen/Qwen2.5-7B-Instruct")
        ...
        if system_prompt:
            message += Qwen25InstructHf.create_system_prompt(system_prompt)

        # Add few-shot examples if provided
        if few_shot_data:
            message += Qwen25InstructHf.create_few_shot_prompt(few_shot_data)

        # Add the main user input (text)
        message += Qwen25InstructHf.create_zero_shot_prompt({"user": text})
