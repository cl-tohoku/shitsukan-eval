...
class LlmJp3InstructHf:
    """
    A wrapper class for using the llm-jp-3 models from Hugging Face with conditional generation.
    ...
    Example usage:
    >>> model = LlmJp3InstructHf(pretrained="llm-jp/llm-jp-3-13b-instruct", device_map={"": 0})
    ...
    def __init__(
        self,
        pretrained: str = "llm-jp/llm-jp-3-13b-instruct",
        revision: str = "main",
        device: str = "cuda",
        ...
        **kwargs,
        ...
        """
        Initializes the LlmJp3InstructHf class by loading the model, processor, and tokenizer.
        ...
        Example:
        >>> model = LlmJp3InstructHf(pretrained="llm-jp/llm-jp-3-13b-instruct", device="cuda")
        """
        super().__init__()
        ...
        # The original chat template has been modified to allow the system prompt to be changed dynamically.
        self._tokenizer.chat_template = "{{bos_token}}{% for message in messages %}{% if message['role'] == 'user' %}{{ '\n\n### 指示:\n' + message['content'] }}{% elif message['role'] == 'system' %}{{ message['content'] }}{% elif message['role'] == 'assistant' %}{{ '\n\n### 応答:\n' + message['content'] + eos_token }}{% endif %}{% if loop.last and add_generation_prompt %}{{ '\n\n### 応答:\n' }}{% endif %}{% endfor %}"
    ...
    def create_system_prompt(system_prompt: str) -> list[dict[str, Any]]:
        ...
        Example:
        >>> prompt = LlmJp3InstructHf.create_system_prompt("You are a helpful assistant.")
    ...
    def create_zero_shot_prompt(zero_shot_data: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        >>> prompt = LlmJp3InstructHf.create_zero_shot_prompt(zero_shot)
        """
        role = "user"
    ...
    def create_few_shot_prompt(few_shot_data: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        >>> prompt = LlmJp3InstructHf.create_few_shot_prompt(few_shot)
        """
        messages = []
    ...
    def generate_response(
        ...
        Example usage:
        >>> model = LlmJp3InstructHf(pretrained="llm-jp/llm-jp-3-13b-instruct")
        ...
        if system_prompt:
            message += LlmJp3InstructHf.create_system_prompt(system_prompt)

        # Add few-shot examples if provided
        if few_shot_data:
            message += LlmJp3InstructHf.create_few_shot_prompt(few_shot_data)

        # Add the main user input (text and images)
        message += LlmJp3InstructHf.create_zero_shot_prompt({"user": text})
        ...
        with torch.no_grad():
            generated_ids = self.model.generate(inputs["input_ids"], **gen_kwargs)
