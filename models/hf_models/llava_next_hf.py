...
class LlavaNextHf:
    ...
    Example usage:
    >>> model = LlavaNextHf(pretrained="llava-hf/llava-v1.6-mistral-7b-hf", device_map={"": 0})
    ...
    def __init__(
        self,
        pretrained: str = "llava-hf/llava-v1.6-mistral-7b-hf",
        image_dir: str = None,
        revision: str = "main",
        device: str = "cuda",
        ...
        **kwargs,
        ...
        """
        Initializes the LlavaNextHf class by loading the model, processor, and tokenizer.
        ...
        Example:
        >>> model = LlavaNextHf(pretrained="llava-hf/llava-v1.6-mistral-7b-hf", device="cuda")
        """
        super().__init__()
    ...
    def create_system_prompt(system_prompt: str) -> list[dict[str, Any]]:
        ...
        Example:
        >>> prompt = LlavaNextHf.create_system_prompt("You are a helpful assistant.")
    ...
    def create_zero_shot_prompt(zero_shot_data: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        >>> prompt = LlavaNextHf.create_zero_shot_prompt(zero_shot)
        """
        messages = []
        role = "user"
        content = []
    ...
    def create_few_shot_prompt(few_shot_data: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        >>> prompt = LlavaNextHf.create_few_shot_prompt(few_shot)
        """
        messages = []
    ...
    def generate_response(
        ...
        Example usage:
        >>> model = LlavaNextHf(pretrained="llava-hf/llava-v1.6-mistral-7b-hf")
        ...
        if images is not None:
            # Count the number of DEFAULT_IMAGE_TOKEN in the text
        ...
        if system_prompt:
            message += LlavaNextHf.create_system_prompt(system_prompt)

        # Add few-shot examples if provided
        if few_shot_data:
            message += LlavaNextHf.create_few_shot_prompt(few_shot_data)

        # Add the main user input (text and images)
        message += LlavaNextHf.create_zero_shot_prompt({"user": text, "images": images})
