...
class Gemma2InstructHf:
    """
    A wrapper class for using the Llama3 models from Hugging Face with conditional generation.
    ...
    Example usage:
    >>> model = Gemma2InstructHf(pretrained="google/gemma-2-2b-jpn-it", device_map={"": 0})
    ...
    def __init__(
        self,
        pretrained: str = "google/gemma-2-2b-jpn-it",
        revision: str = "main",
        device: str = "cuda",
        ...
        **kwargs,
        ...
        """
        Initializes the Gemma2InstructHf class by loading the model, processor, and tokenizer.
        ...
        Example:
        >>> model = Gemma2InstructHf(pretrained="google/gemma-2-2b-jpn-it", device="cuda")
        """
        super().__init__()
    ...
    def create_zero_shot_prompt(zero_shot_data: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        >>> prompt = Gemma2InstructHf.create_zero_shot_prompt(zero_shot)
        """
        role = "user"
    ...
    def create_few_shot_prompt(few_shot_data: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        >>> prompt = Gemma2InstructHf.create_few_shot_prompt(few_shot)
        """
        messages = []
    ...
    def generate_response(
        ...
        Example usage:
        >>> model = Gemma2InstructHf(pretrained="google/gemma-2-2b-jpn-it")
        ...
        message = []
        updated_few_shot_data = copy.deepcopy(few_shot_data)

        if system_prompt:
            if few_shot_data:
                # Add system prompt and few-shot examples
                updated_few_shot_data[0]["user"] = system_prompt + "\n\n" + few_shot_data[0]["user"]
                message += Gemma2InstructHf.create_few_shot_prompt(updated_few_shot_data)
                # Add the main user input
                message += Gemma2InstructHf.create_zero_shot_prompt({"user": text})
            else:
                # Add system prompt and the main user input
                message += Gemma2InstructHf.create_zero_shot_prompt({"user": system_prompt + "\n\n" + text})
        elif few_shot_data:
            # Add few-shot examples
            message += Gemma2InstructHf.create_few_shot_prompt(few_shot_data)
            # Add the main user input
            message += Gemma2InstructHf.create_zero_shot_prompt({"user": text})
        else:
            # Add the main user input
