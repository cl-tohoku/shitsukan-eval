...
class LlavaOnevisionHf:
    """
    A wrapper class for using the Llava Onevision models from Hugging Face with conditional generation.
    ...
    >>> model = LlavaOnevisionHf(pretrained="llava-hf/llava-onevision-qwen2-7b-ov-hf", device_map={"": 0})
    ...
    def create_system_prompt(system_prompt: str) -> list[dict[str, Any]]:
        ...
        Example:
        >>> prompt = LlavaOnevisionHf.create_system_prompt("You are a helpful assistant.")
    ...
    def create_zero_shot_prompt(zero_shot_data: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        >>> prompt = LlavaOnevisionHf.create_zero_shot_prompt(zero_shot)
        """
        messages = []
        role = "user"
        content = []
    ...
    def create_few_shot_prompt(few_shot_data: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        >>> prompt = LlavaOnevisionHf.create_few_shot_prompt(few_shot)
        """
        messages = []
    ...
    def generate_response(
        ...
        if system_prompt:
            message += LlavaOnevisionHf.create_system_prompt(system_prompt)

        # Add few-shot examples if provided
        if few_shot_data:
            message += LlavaOnevisionHf.create_few_shot_prompt(few_shot_data)

        # Add the main user input (text and images)
        message += LlavaOnevisionHf.create_zero_shot_prompt({"user": text, "images": images})
